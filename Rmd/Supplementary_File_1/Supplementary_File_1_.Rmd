...
opts_knit$set(width=75,
              root.dir = '/Users/luketheduke2/Documents/Academic/projects/snowy_plover_eggs')
```
...
show_col(wsj_pal(palette = "rgby")(4))
ggthemes_data$wsj
...
ceuta_egg_chick_female_data <- 
  readRDS("data/ceuta_egg_chick_female_data_cs.rds")
...
load("output/key/BaSTA_age_estimates_2006-2020.rds")
...
egg_widths_plot <- 
  ...
  geom_jitter(aes(x = width/10, y = 428), 
              ...
              color = brewer.pal(8, "Set1")[c(2)],
              height = 8, alpha = 0.3) +
  geom_histogram(alpha = 0.75, aes(width/10), 
                 fill = brewer.pal(8, "Set1")[c(2)], 
                 binwidth = 0.02) +
  luke_theme +
...
```{r, echo=FALSE}
ceuta_egg_chick_female_data %>%
  select(ID, ring, year, volume_cm, est_age_t_deviation, first_age_t, last_age_t, avg_ad_tarsi, laydate_deviation, first_laydate) %>% 
  distinct() %>% 
...
```{r, echo=FALSE}
#### Results and data----
load("output/Stats_eggv_mod_cs.rds")
...
R2Table <- 
  bind_rows(stats_eggv_mod_cs$partR2m$R2,
            stats_eggv_mod_cs$partR2c$R2[1,]) %>% 
...
allCoefs_mod <-
  allCoefs_mod[c(5, 9, 1:4, 6:8, 10, 18, 16:17, 11:15, 19:30), ]
...
eggv_mod_forest_plot_partR2 <-
  ...
  scale_y_discrete(labels = c("Between ind. seasonality" = expression("Between ind. seasonality"),
                              ...
                              "Clutch size (< 3 eggs)" = expression("Clutch size (< 3 eggs)"),
...
```{r, echo=FALSE}
ceuta_egg_chick_female_data_cs = ceuta_egg_chick_female_data
```


### *Figure S7L (<ins>L</ins>ower panel in manuscript)*
...
eggv_mod_tarsus_fits <- 
  as.data.frame(effect(term = "avg_ad_tarsi", mod = stats_eggv_mod_cs$mod_poly, 
...
eggv_first_age_boot_out_melt <- reshape2::melt(t(est_age_boot_out[["mod_eggv_boot_first_age_fits"]]))
colnames(eggv_first_age_boot_out_melt) <- c("first_age", "iteration", "volume_cm")

eggv_mod_first_age_fits <- 
  as.data.frame(effect(term = "first_age_t", mod = stats_eggv_mod_cs$mod_poly,
...
eggv_last_age_boot_out_melt <- reshape2::melt(t(est_age_boot_out[["mod_eggv_boot_last_age_fits"]]))
colnames(eggv_last_age_boot_out_melt) <- c("last_age", "iteration", "volume_cm")

eggv_mod_last_age_fits <- 
  as.data.frame(effect(term = "last_age_t", mod = stats_eggv_mod_cs$mod_poly, 
...
eggv_age_dev_boot_out_melt <- reshape2::melt(t(est_age_boot_out[["mod_eggv_boot_age_dev_fits"]]))
colnames(eggv_age_dev_boot_out_melt) <- c("est_age_t_deviation", "iteration", "volume_cm")

eggv_mod_age_dev_fits <- 
  as.data.frame(effect(term = "poly(est_age_t_deviation, 2)", mod = stats_eggv_mod_cs$mod_poly, 
...
eggv_mod_within_fits <- 
  as.data.frame(effect("laydate_deviation", stats_eggv_mod_cs$mod_poly, 
...
# extract the fitted values of the polynomial season effect
eggv_mod_date_fits <- 
  as.data.frame(effect("poly(first_laydate, 2)", stats_eggv_mod_cs$mod_poly, 
...
load("output/Stats_polyandry_age_avg_ev.rds")
...
poly_peak2[[1]] - mono_peak2[[1]]
poly_peak1[[1]] - mono_peak2[[1]]
```
...
# Fixed effect sizes (standardized)
fixef_bw_Table <- 
  stats_polyandry_age_ev$partR2m$BW %>% 
  # dplyr::select(term, estimate, CI_lower, CI_upper) %>% 
  as.data.frame() %>% 
  mutate(stat = "fixed_bw") %>% 
  rename(conf.low = CI_lower,
...
# Random effects variances
R2Table <- 
  bind_rows(stats_polyandry_age_ev$partR2m$R2,
            stats_polyandry_age_ev$partR2c$R2[1,]) %>%   
  dplyr::select(term, estimate, CI_lower, CI_upper) %>% 
  as.data.frame() %>% 
  mutate(stat = "partR2") %>% 
  rename(conf.low = CI_lower,
...
# Adjusted repeatabilities
coefRptTable <- 
  stats_polyandry_age_ev$rptR$R["R_org", ] %>% 
  dplyr::select(-Fixed) %>%
  t() %>% 
  as.data.frame() %>% 
  bind_cols(stats_polyandry_age_ev$rptR$CI_emp$CI_org[c("ring", "year"),]) %>% 
  rownames_to_column("term") %>% 
...
# re-organize model components for table
allCoefs_mod <-
  allCoefs_mod[c(1, 5, 2, 3, 4, 6, 12, 7, 11, 8:10, 13:19), ]
...
rene_peak2[[1]] - sing_peak2[[1]]
rene_peak1[[1]] - sing_peak2[[1]]
```
```{r, echo=FALSE}
#### Table of effect sizes ----
# Retrieve sample sizes
...
fixef_bw_Table <-
  ...
  filter(str_detect(term, negate = TRUE, "SD")) %>% 
...
### *Figure 3R (<ins>R</ins>ight panel in manuscript)*
...
### *Figure S7T (<ins>T</ins>op panel in manuscript)*
...
### *Figure S8L (<ins>L</ins>ower row in manuscript)*
<font size = "2">Visualization of the effect of uncertainty in the age estimate provided by BaSTA on age-dependent trends in lay date dynamics. Black trends show the model predictions for the 1000 bootstraps of the BaSTA age estimate posteriors (see Methods). A) within-individual trend of the 'age-deviation' score – as expected, this measure is not impacted by uncertainty in the BaSTA age estimate because they are centered for each individual (i.e., the absolute age is irrelevant). B) between-individual trend of the 'age at first breeding' (i.e., selective appearance). C) between-individual trend of the 'age at last breeding' (i.e., selective disappearance). Yellow trends and grey ribbons visualize the 95% CI of the model predictions when using the mean age estimate provided by BaSTA (i.e., the effect sizes of the 'lay date' models shown in Fig. 3R).</font>
```{r, echo=FALSE,fig.dim=c(8,4)}
load("output/age_estimate_uncertainty_bootstraps.rds")
...
```{r, echo=FALSE}
load(file = "output/known_age_basta_age_estimate_boot_out.rds")
load(file = "data/BaSTA_checked_life_table_females_2006-2020.rds")
load(file = "data/BaSTA_ID_ring_key.rds")
```
